	...
	RETURN QUERY 
        SELECT 
			SUM(
				...
				(CASE WHEN EXISTS(SELECT * FROM osc.tb_representante_conselho INNER JOIN osc.tb_participacao_social_conselho ON tb_representante_conselho.id_participacao_social_conselho = tb_participacao_social_conselho.id_conselho WHERE tb_participacao_social_conselho.id_osc = osc AND tx_nome_representante_conselho IS NOT null) THEN peso_campo ELSE 0 END) + 
