...
SELECT
	tb_osc_parceira_projeto.id_osc_parceira_projeto,
	tb_osc_parceira_projeto.id_projeto,
	tb_osc_parceira_projeto.id_osc,
	(
		SELECT 
			COALESCE(NULLIF(TRIM(tb_dados_gerais.tx_nome_fantasia_osc), ''), tb_dados_gerais.tx_razao_social_osc) 
		FROM 
			osc.tb_dados_gerais 
		WHERE 
			tb_dados_gerais.id_osc = tb_osc_parceira_projeto.id_osc
	) AS tx_nome_osc_parceira_projeto,
