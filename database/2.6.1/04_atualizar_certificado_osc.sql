...
BEGIN
	...
	LOOP
		...
		ELSIF tipo_busca = 3 THEN
			IF objeto.cd_municipio IS NOT null THEN
				SELECT INTO dado_anterior *
				FROM osc.tb_certificado
				WHERE cd_certificado = objeto.cd_certificado
				AND cd_municipio = objeto.cd_municipio
				AND cd_uf IS null
				AND id_osc = osc;
			ELSIF objeto.cd_uf IS NOT null THEN
				SELECT INTO dado_anterior *
				FROM osc.tb_certificado
				WHERE cd_certificado = objeto.cd_certificado
				AND cd_municipio IS null
				AND cd_uf = objeto.cd_uf
				AND id_osc = osc;
			ELSE
				SELECT INTO dado_anterior *
				FROM osc.tb_certificado
