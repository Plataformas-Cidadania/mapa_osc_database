...
ALTER TABLE osc.tb_dados_gerais 
	ADD COLUMN cd_classe_atividade_economica_osc CHARACTER VARYING, 
	ADD CONSTRAINT fk_cd_classe_atividade_economica_osc
	FOREIGN KEY (cd_classe_atividade_economica_osc) 
	REFERENCES syst.dc_classe_atividade_economica(cd_classe_atividade_economica),
    ADD COLUMN ft_classe_atividade_economica_osc TEXT,
    DROP CONSTRAINT fk_cd_subclasse_atividade_economica_osc,
    DROP COLUMN cd_subclasse_atividade_economica_osc,
    DROP COLUMN ft_subclasse_atividade_economica_osc;
...
AS
 SELECT tb_osc.id_osc,
    ...
    rtrim(replace((((((((((((COALESCE(tb_localizacao.tx_endereco, '|'::text) || ', '::text) || COALESCE(tb_localizacao.nr_localizacao::text, '|'::text)) || ', '::text) || COALESCE(tb_localizacao.tx_endereco_complemento, '|'::text)) || ', '::text) || COALESCE(tb_localizacao.tx_bairro, '|'::text)) || ', '::text) || COALESCE((( SELECT ed_municipio.edmu_nm_municipio AS tx_municipio
    ...
    ( SELECT dc_classe_atividade_economica.tx_nome_classe_atividade_economica
           FROM syst.dc_classe_atividade_economica
          WHERE dc_classe_atividade_economica.cd_classe_atividade_economica = tb_dados_gerais.cd_classe_atividade_economica_osc) AS tx_nome_atividade_economica
   FROM osc.tb_osc
...
ALTER TABLE portal.vw_busca_resultado
    OWNER TO postgres;


CREATE UNIQUE INDEX ix_vw_busca_resultado
    ON portal.vw_busca_resultado USING btree
    (id_osc)
    TABLESPACE pg_default;    
