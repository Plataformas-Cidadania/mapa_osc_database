...
BEGIN 
	...
	RETURN QUERY 
		...
		FROM (
			...
			FROM (
				SELECT 
					ARRAY_AGG('{"rotulo": "' || a.natureza_organizacao::TEXT || '", "valor": ' || a.quantidade::TEXT || '}')::TEXT AS dados 
				FROM (
					SELECT 
						tb_cnes.ds_natureza_organizacao AS natureza_organizacao, 
					...
					GROUP BY tb_cnes.ds_natureza_organizacao
				) AS a
			) AS b
		) AS c;
END;
