DROP FUNCTION IF EXISTS portal.atualizar_objetivos_osc(fonte TEXT, identificador NUMERIC, tipo_identificador TEXT, data_atualizacao TIMESTAMP, json JSONB, nullvalido BOOLEAN, delete_valido BOOLEAN, erro_log BOOLEAN, id_carga INTEGER, tipo_busca INTEGER);
...
BEGIN
	...
	IF delete_valido THEN
		FOR objeto IN (SELECT * FROM osc.tb_objetivo_osc WHERE id_osc = osc)
		LOOP
