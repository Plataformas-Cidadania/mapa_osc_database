...
CREATE function portal.obter_osc_recursos(param text)
    returns TABLE
            (
                resultado jsonb,
                mensagem  text,
        ...
        resultado := (
            jsonb_build_object(
                ...
                FROM (SELECT jsonb_build_object(
                                     ...
                                     TRIM(TO_CHAR(nr_valor_recursos_osc, '99999999999999999999D99')),
                    ...
                    'recursos_outros', (
                        ...
                        FROM (
                                 SELECT jsonb_build_object(
                                                ...
                                                SUBSTRING(dt_ano_recursos_outro_osc::text FROM 1 FOR 4),
                                                'ft_ano_recursos_outro_osc', ft_ano_recursos_outro_osc,
                                                'nr_valor_recursos_outro_osc',
                                                TRIM(TO_CHAR(nr_valor_recursos_outro_osc, '99999999999999999999D99')),
                                                'ft_valor_recursos_outro_osc', ft_valor_recursos_outro_osc
                                            ) as l

                                 FROM osc.tb_recursos_outro_osc
                                 WHERE id_osc = param::INTEGER
                                 ORDER BY dt_ano_recursos_outro_osc) as x
                    )
                )
            );
...
alter function portal.obter_osc_recursos(text) owner to i3geo;