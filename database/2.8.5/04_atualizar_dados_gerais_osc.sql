create or replace function portal.atualizar_dados_gerais_osc(fonte text, identificador numeric, tipo_identificador text, data_atualizacao timestamp without time zone, json jsonb, null_valido boolean, delete_valido boolean, erro_log boolean, id_carga integer, tipo_busca integer) returns TABLE(mensagem text, flag boolean)
	language plpgsql
as $$
DECLARE
	nome_tabela TEXT;
...
BEGIN
	...
	ELSE
        ...
        nome_campo := 'tx_nome_fantasia_osc';
		IF (SELECT a.flag FROM portal.verificar_dado(dado_anterior.tx_nome_fantasia_osc::TEXT, dado_anterior.ft_nome_fantasia_osc, objeto.tx_nome_fantasia_osc::TEXT, fonte_dados.prioridade, nome_campo, null_valido) AS a) THEN
	...
	objetivos = COALESCE((json->>'objetivo_metas')::JSONB, null::JSONB);
	raise notice '%', objetivos;
...
alter function portal.atualizar_dados_gerais_osc(text, numeric, text, timestamp, jsonb, boolean, boolean, boolean, integer, integer) owner to postgres;
