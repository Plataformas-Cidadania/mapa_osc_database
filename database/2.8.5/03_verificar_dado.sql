create or replace function portal.verificar_dado(dado_anterior text, fonte_dado_anterior text, dado_posterior text, dado_posterior_prioridade integer, nome_campo text, nullvalido boolean) returns TABLE(flag boolean)
	language plpgsql
as $$
DECLARE
	fonte_ajustada TEXT;
BEGIN
	...
	IF ( ( nullvalido = true AND dado_anterior <> dado_posterior) OR (nullvalido = false AND dado_anterior <> dado_posterior AND (dado_posterior::TEXT = '') IS false) )
       ...
       OR (SELECT ce.editavel FROM syst.tb_campos_editaveis ce WHERE ce.nome_campo = nome_campo::text)
    THEN
		flag := true;
	END IF;
