...
BEGIN	
	...
	RETURN QUERY
		EXECUTE 
			...
			WHERE vw_spat_estado.eduf_nm_uf_ajustado ILIKE TRANSLATE(LOWER(UNACCENT(''' || param::TEXT || '%''::TEXT)), '' _-'', '''')
			OR vw_spat_estado.eduf_sg_uf_ajustado = TRANSLATE(LOWER(''' || param::TEXT || '''), '' _-'', '''')
			ORDER BY vw_spat_estado.eduf_nm_uf DESC ' || query_limit;
