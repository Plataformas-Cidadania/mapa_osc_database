...
BEGIN
	...
	IF nao_possui IS NOT null THEN
		IF nao_possui IS true THEN 
			SELECT INTO record_externo * FROM portal.atualizar_osc(fonte, osc.id_osc, 'id_osc', data_atualizacao, '{"bo_nao_possui_projeto": true}'::JSONB, false::BOOLEAN, delete_valido, erro_log, id_carga);
		ELSE 
			SELECT INTO record_externo * FROM portal.atualizar_osc(fonte, osc.id_osc, 'id_osc', data_atualizacao, '{"bo_nao_possui_projeto": false}'::JSONB, false::BOOLEAN, delete_valido, erro_log, id_carga);
		END IF;
