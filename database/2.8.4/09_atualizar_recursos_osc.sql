-- FUNCTION: portal.atualizar_recursos_osc(text, numeric, text, timestamp without time zone, jsonb, boolean, boolean, boolean, integer, integer)

-- DROP FUNCTION portal.atualizar_recursos_osc(text, numeric, text, timestamp without time zone, jsonb, boolean, boolean, boolean, integer, integer);

CREATE OR REPLACE FUNCTION portal.atualizar_recursos_osc(
	fonte text,
...
BEGIN
	...
	IF delete_valido THEN
		IF ano IS null THEN
			FOR objeto IN (SELECT * FROM osc.tb_recursos_osc WHERE id_osc = osc AND id_recursos_osc != ALL(dado_nao_delete))
			LOOP
			...
			END LOOP;
		ELSE
			FOR objeto IN (SELECT * FROM osc.tb_recursos_osc WHERE id_osc = osc AND id_recursos_osc != ALL(dado_nao_delete) AND dt_ano_recursos_osc = ano)
			LOOP
				IF (SELECT a.flag FROM portal.verificar_delete(fonte_dados.prioridade, ARRAY[objeto.ft_fonte_recursos_osc, objeto.ft_ano_recursos_osc, objeto.ft_valor_recursos_osc, objeto.ft_nao_possui]) AS a) THEN
					DELETE FROM osc.tb_recursos_osc WHERE id_recursos_osc = objeto.id_recursos_osc;
					PERFORM * FROM portal.inserir_log_atualizacao(nome_tabela, osc, fonte, data_atualizacao, row_to_json(objeto), null,id_carga);
