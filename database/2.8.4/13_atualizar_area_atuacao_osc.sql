-- FUNCTION: portal.atualizar_area_atuacao_osc(text, numeric, text, timestamp without time zone, jsonb, boolean, boolean, boolean, integer, integer)

-- DROP FUNCTION portal.atualizar_area_atuacao_osc(text, numeric, text, timestamp without time zone, jsonb, boolean, boolean, boolean, integer, integer);

CREATE OR REPLACE FUNCTION portal.atualizar_area_atuacao_osc(
	fonte text,
...
BEGIN
	...
	LOOP
		...
		ELSIF tipo_busca = 2 AND objeto.cd_subarea_atuacao IS NOT NULL THEN
			SELECT INTO dado_anterior *
			FROM osc.tb_area_atuacao
		...
		ELSIF tipo_busca = 2 AND objeto.cd_subarea_atuacao IS NULL THEN
			SELECT INTO dado_anterior *
			FROM osc.tb_area_atuacao
			WHERE cd_area_atuacao = objeto.cd_area_atuacao
			AND cd_subarea_atuacao IS NULL
			AND id_osc = osc;
...
ALTER FUNCTION portal.atualizar_area_atuacao_osc(text, numeric, text, timestamp without time zone, jsonb, boolean, boolean, boolean, integer, integer)
    OWNER TO postgres;
