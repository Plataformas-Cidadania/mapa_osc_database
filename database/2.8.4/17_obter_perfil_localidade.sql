-- FUNCTION: analysis.obter_perfil_localidade(integer)

-- DROP FUNCTION analysis.obter_perfil_localidade(integer);

CREATE OR REPLACE FUNCTION analysis.obter_perfil_localidade(
		...
		FROM (
			SELECT
				...
				(
					...
					FROM (
						SELECT
							'Vínculos formais' AS label,
							vinculos AS value
						FROM analysis.vw_perfil_localidade_trabalhadores
						WHERE localidade = id_localidade::TEXT
						UNION
						SELECT
							'Vínculos formais de pessoas com deficiência' AS label,
...
ALTER FUNCTION analysis.obter_perfil_localidade(integer)
    OWNER TO i3geo;
