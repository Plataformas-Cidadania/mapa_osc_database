-- FUNCTION: portal.atualizar_relacoes_trabalho_osc(text, numeric, text, timestamp without time zone, jsonb, boolean, boolean, integer)

-- DROP FUNCTION portal.atualizar_relacoes_trabalho_osc(text, numeric, text, timestamp without time zone, jsonb, boolean, boolean, integer);

CREATE OR REPLACE FUNCTION portal.atualizar_relacoes_trabalho_osc(
	fonte text,
...
BEGIN
	...
	ELSE
		...
		IF (SELECT a.flag FROM portal.verificar_dado(dado_anterior.nr_trabalhadores_deficiencia::TEXT, dado_anterior.ft_trabalhadores_deficiencia::TEXT, objeto.nr_trabalhadores_deficiencia::TEXT, fonte_dados.prioridade, null_valido) AS a) THEN
