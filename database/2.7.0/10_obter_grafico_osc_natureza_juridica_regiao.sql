						...
						FROM (
							SELECT * FROM (
								SELECT 
									...
									COUNT(*) AS valor, 
									(
									TRIM(ARRAY_AGG(DISTINCT TRIM(tb_dados_gerais.ft_classe_atividade_economica_osc)) FILTER (WHERE (TRIM(tb_dados_gerais.ft_classe_atividade_economica_osc) = '') IS false)::TEXT, '{}') || ',' || 
