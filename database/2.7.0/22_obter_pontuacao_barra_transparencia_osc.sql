DROP FUNCTION IF EXISTS portal.obter_pontuacao_barra_transparencia_osc(INTEGER) CASCADE;

CREATE OR REPLACE FUNCTION portal.obter_pontuacao_barra_transparencia_osc(osc INTEGER) RETURNS TABLE (
...
BEGIN 
	...
	pontuacao_dados_gerais := (SELECT pontuacao FROM portal.obter_barra_transparencia_dados_gerais(osc));
	pontuacao_area_atuacao := (SELECT pontuacao FROM portal.obter_barra_transparencia_area_atuacao(osc));
	pontuacao_descricao := (SELECT pontuacao FROM portal.obter_barra_transparencia_descricao(osc));
	pontuacao_titulos_certificacoes := (SELECT pontuacao FROM portal.obter_barra_transparencia_titulos_certificacoes(osc));
	pontuacao_relacoes_trabalho_governanca := (SELECT pontuacao FROM portal.obter_barra_transparencia_relacoes_trabalho_governanca(osc));
	pontuacao_espacos_participacao_social := (SELECT pontuacao FROM portal.obter_barra_transparencia_espacos_participacao_social(osc));
	pontuacao_projetos_atividades_programas := (SELECT pontuacao FROM portal.obter_barra_transparencia_projetos_atividades_programas(osc));
	pontuacao_fontes_recursos := (SELECT pontuacao FROM portal.obter_barra_transparencia_fontes_recursos(osc));
