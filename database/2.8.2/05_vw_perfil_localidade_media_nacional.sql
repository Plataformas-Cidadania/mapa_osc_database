create materialized view analysis.vw_perfil_localidade_media_nacional as
    SELECT 'maior_natureza_juridica'::text AS tipo_dado,
           ...
           ((max(a.quantidade_osc) / (SELECT sum(c.quantidade_oscs) AS sum
                                      FROM analysis.vw_perfil_localidade_natureza_juridica c)) *
            (100)::numeric)                AS valor
...
alter materialized view analysis.vw_perfil_localidade_media_nacional owner to b116908948;

create index ix_tipo_dado_vw_perfil_localidade_media_nacional
    on analysis.vw_perfil_localidade_media_nacional (tipo_dado);