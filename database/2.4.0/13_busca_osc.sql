...
BEGIN 
	...
	RETURN QUERY 
		EXECUTE 
			...
			WHERE 
				vw_busca_osc.cd_identificador_osc::TEXT ILIKE ''%'' || LTRIM(''' || param::TEXT || ''', ''0'') || ''%'' 
			OR 
			(
				document @@ to_tsquery(''portuguese_unaccent'', ''' || param::TEXT || ''') 
				AND 
				(
					similarity(vw_busca_osc.tx_razao_social_osc::TEXT, ''' || param::TEXT || ''') > ' || similarity_result::DOUBLE PRECISION || ' 
					OR 
