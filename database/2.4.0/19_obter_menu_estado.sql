	...
	RETURN QUERY
		EXECUTE 
			...
			FROM 
				spat.vw_spat_estado
			WHERE 
				vw_spat_estado.eduf_nm_uf_ajustado ILIKE ''' || param::TEXT || '%'' 
			OR 
				vw_spat_estado.eduf_sg_uf_ajustado = ''' || param::TEXT || '''
			ORDER BY 
				GREATEST(
					similarity(vw_spat_estado.eduf_nm_uf_ajustado, ''' || param::TEXT || '''), 
					similarity(vw_spat_estado.eduf_sg_uf_ajustado, ''' || param::TEXT || ''')
				) DESC ' || query_limit;
