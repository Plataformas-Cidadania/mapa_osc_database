...
CREATE MATERIALIZED VIEW osc.vw_busca_osc AS 
	SELECT tb_osc.id_osc,
		...
		TRIM(TRANSLATE(TRANSLATE(TRANSLATE(tb_dados_gerais.tx_razao_social_osc, '/,\,|,:,#,@,$,&,!,?,(,),[,]', ''), '.'::TEXT, ','::TEXT), '_'::TEXT, ' '::TEXT)) AS tx_razao_social_osc,
		NULLIF(TRIM(TRANSLATE(TRANSLATE(TRANSLATE(tb_dados_gerais.tx_nome_fantasia_osc, '/,\,|,:,#,@,$,&,!,?,(,),[,]', ''), '.'::TEXT, ','::TEXT), '_'::TEXT, ' '::TEXT)), ''::TEXT) AS tx_nome_fantasia_osc,
		(
			setweight(to_tsvector('portuguese_unaccent'::regconfig, COALESCE(TRIM(TRANSLATE(TRANSLATE(TRANSLATE(tb_dados_gerais.tx_razao_social_osc, '/,\,|,:,#,@,$,&,!,?,(,),[,]', ''), '.'::TEXT, ','::TEXT), '_'::TEXT, ' '::TEXT)), ''::TEXT)), 'A'::"char") || 
			setweight(to_tsvector('portuguese_unaccent'::regconfig, COALESCE(TRIM(TRANSLATE(TRANSLATE(TRANSLATE(tb_dados_gerais.tx_nome_fantasia_osc, '/,\,|,:,#,@,$,&,!,?,(,),[,]', ''), '.'::TEXT, ','::TEXT), '_'::TEXT, ' '::TEXT)), ''::TEXT)), 'B'::"char")
		) AS document,
		(
...
ALTER TABLE osc.vw_busca_osc OWNER TO postgres;
